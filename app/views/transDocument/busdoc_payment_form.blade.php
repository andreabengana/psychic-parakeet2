                      ...
                      <table style="border-spacing: 4px;border-collapse: separate;">
                        <tbody>
                          <tr>
                            ...
                            <td><span class="label label-primary"><label for="docNo">{{$request[0] -> BusRequestID}}</label></span></td>
                          </tr>
                          <tr>
                      ...
                      <table style="border-spacing: 4px;border-collapse: separate;">
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td><label for="docby">{{$request[0] -> BusRequestor}}</label></td>
                          </tr>
                          <tr>
                            <td class="pull-right">Requested for: </td>
                            <td><label for="docfor">{{$request[0] -> BusinessName}}</label></td>
                          </tr>
                        </tbody>
                      </table>
                      </div>
                      ...
                      <table  style="border-spacing: 17px;border-collapse: separate;">
                        <tbody>
                          <tr bgcolor="#ffffb3">
                            ...
                            <td><input class="form-control" type="number" step="any" id="subtotal" name="subtotal" value="{{$req -> DocumentFee}}" disabled >
                            </td>
                          </tr>
                          <tr bgcolor="#ffffb3">
      ...
      $(document).ready(function(){
        ...
        $('#btnPrint').click(function(){
          ...
          $.ajax({
            ...
            success: function(data)
            {
              window.location.href = "{{URL::to('busdocReceipt')}}?reqID=" + $('label[for="docNo"]').html() + "";
            }
          }).error(function(ts){
            alert(ts.responseText);
          });
        ...
        $('#btnSubmit').click(function(){
          $.ajax({
            ...
            success:function(data){
              window.location.href = "{{URL::to('docPayment')}}?sort=bus";
            }           
          });
